...
<table>                           
		...
		<?php
                ...
                $sql="select title,sendername,des,mID from pic";
                $result=mysqli_query($link, $sql);
				$i=1;		
               while($row=mysqli_fetch_row($result)){
                $_SESSION['mypicname{$i}']=$row[0];
                $title=$row[0];
				$sender=$row[1];
                $destination=$row[2];
				$mid=$row[3];
                ...
                echo "<tr>
                         ...
                         "<td><a href='".$destination."' target='_blank'>click to see picture</a></td>".
						 "<td><a href='deletepic.php?mid=".$mid."'>delete</a>
                     </tr>";
                $i++;
            }
